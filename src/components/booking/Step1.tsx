...
const Step1 = ({ setStep, data, _data, set_data }: any) => {
    ...
    const [allRoutes, setAllRoutes] = useState<null | any[]>(null)
    const [selectedBusId, setSelectedBusId] = useState<null | string>(null)
    ...
    const getRoutes = async () => {
        ...
        try {
            ...
            setAllRoutes(routeResp?.data?.data)
    ...
    useEffect(() => {
        ...
        if (!selectedBusId || selectedBusId?.trim() === "" || !allRoutes) return

        setRoutes(null)

        const _routes = [...allRoutes]?.filter((route: any) => route?.bus === selectedBusId)
        ...
        <div className={`w-full bg-[${isDarkMode ? "#0a101d" : "#fff"}] flex flex-col gap-4 p-8 pt-16`}>
            ...
            <div className="w-full flex justify-between items-center gap-4 p-4 mt-4">
                ...
                <div
                    ...
                    {
                        buses?.length ? buses?.map((bus: any, i: number) => (
                            <div
                                onClick={() => setSelectedBusId(bus?._id)}
                                key={i} className={`w-[300px] p-2 text-[${isDarkMode ? "#fff" : "#0a101d"}] text-[32px] font-bold flex justify-center items-center text-center cursor-pointer`}>{`${bus?.name}>`}</div>
                        )) : null
                    }
                </div>
                <p className="text-[#29ABE2] text-[48px] cursor-pointer"
            ...
            <div className="flex flex-col gap-4 p-4">
                {
                    !routes ?
                        <div className="w-full flex justify-center items-center p-4 mt-4">
                            <p className={`text-[${isDarkMode ? "#fff" : "#444"}] w-full text-center text-[40px] h-screen`}>
                                {isSelectedEnglish ? "Loading..." : "جاری ہے..."}
                            </p>
                        </div>
                        : routes?.length ? routes?.map((route: any, i: number) => (
                            <DropAccordion
                                key={i}
